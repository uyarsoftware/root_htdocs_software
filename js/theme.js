...
(function ($) {
  ...
  if ($("#contactForm").length > 0) {
    $("#contactForm").on("submit", function (e) {
      ...
      if (required === 0) {
        $.ajax({
          type: "POST",
          url: "ajax/php/mail-uyarsscouk.php",
          data: {
            uyarss_f_name: f_name,
            uyarss_l_name: l_name,
            uyarss_email: email,
            uyarss_phone: phone,
            uyarss_message: message
          },
  ...
  if ($(".color_settings").length > 0) {
    ...
    /*--------------------------------------------------------
    / 19. Subscribe From Submit - atakanau
    /----------------------------------------------------------*/
  if ($(".subscribe").length > 0) {
    $(".subscribe").on("submit", function (e) {
      e.preventDefault();
      $('.subscribe').find('button').html("<span>Processsing...</span>");
      ...
      if (required === 0) {
        $.ajax({
          ...
          data: {
            uyarss_email: email,
            uyarss_message: "Subscribe"
          },
          success: function (data) {
            //alert(data);
            $('.subscribe').find('button').html("<span>Done!</span>");
            $(".subscribe input, .subscribe textarea").val("");
            setTimeout(function () {
              $('.subscribe').find('button').html("<span>Subscribe now</span>");
            }, 2500);
          }
        });
      } else {
        $('.subscribe').find('button').html("<span>Failed!</span>");
      }
    });

    $('.subscribe').find('input').on("keyup", function () {
      $(this).removeClass("reqError");
    });
  }
